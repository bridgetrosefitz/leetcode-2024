...
function pacificAtlantic(heights) {
  ...
  function dfs(row, col, ocean, prevHeight) {
    ...
    if (canReach[ocean].has(cell)) return true;
    if (isAtOcean[ocean](row, col)) {
      canReach[ocean].add(cell);
      return true;
    }

    // if (visited.has(cell)) return false; // I omitted this at first. Can we go over the path of the algo?
    // visited.add(cell);
  ...
  return Array.from(canReach.pac)
    .map(cell => decode(cell))
    .filter(cell => canReach.atl.has(encode(cell[0], cell[1])));
}
