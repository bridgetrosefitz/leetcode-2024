...
BinarySearchTree.prototype.findInOrderSuccessor = function (inputNode) {
  ...
  let curr = inputNode.right; // handle if there is no right child
  let prev = null;
  while (curr) {
    prev = curr;
    curr = curr.left;
  ...
  //https://docs.google.com/spreadsheets/d/1UHEJcf8OitTLQDYtwUS29H6UDFZtIwZ4sW3mie9eN5c/edit?usp=sharing
  // dfs
  // if node is null
  // pass the left value back up
  // for the first recursive call, go to the right child of inputNode
  // go as far left as possible (i.e. if there is still a left child, recurse again)

  // handle if input node has no children (e.g. 14)
};
